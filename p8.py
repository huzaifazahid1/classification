...
def _tokenkey(s: str) -> str:
    """
    [FIX-09] Order-independent key for a label: lowercase the words, sort them,
    join them back. 'Abstract Geometric' and 'Geometric Abstract' both become
    'abstract geometric', so a reordered label matches instead of being
    discarded. difflib scores those two below the 0.75 cutoff because it
    compares character sequences, not word sets.
    """
    return " ".join(sorted(s.lower().replace("&", "and").split()))


# (category, token key) → canonical subcategory, for word-order matching
# inside one category.
_SUB_TOKENS: Dict[Tuple[str, str], str] = {
    (cat, _tokenkey(sub)): sub
    for cat, subs in TAXONOMY.items()
    for sub in subs
}

# [FIX-10] Which category owns a given subcategory name. Only names that are
# unique across the whole taxonomy are listed: 19 names legitimately belong to
# two categories ('Beach' to Nature and Travel, 'Doctor' to People and
# Medical), and for those there is no single correct owner to move an image to,
# so they are deliberately excluded rather than guessed at.
_SUB_OWNER: Dict[str, str] = {}
_SUB_TOKEN_OWNER: Dict[str, str] = {}
_SUB_TOKEN_OWNER_NAME: Dict[str, str] = {}
for _cat, _subs in TAXONOMY.items():
    for _sub in _subs:
        _tk = _tokenkey(_sub)
        if _sub in _SUB_OWNER:
            _SUB_OWNER[_sub] = ""            # ambiguous — never move on this one
        else:
            _SUB_OWNER[_sub] = _cat
        if _tk in _SUB_TOKEN_OWNER:
            _SUB_TOKEN_OWNER[_tk] = ""
        else:
            _SUB_TOKEN_OWNER[_tk] = _cat
            _SUB_TOKEN_OWNER_NAME[_tk] = _sub
_SUB_OWNER = {k: v for k, v in _SUB_OWNER.items() if v}
_SUB_TOKEN_OWNER = {k: v for k, v in _SUB_TOKEN_OWNER.items() if v}
...
class AIEngine:
    ...
    def _parse_subcategory(self, raw: str, category: str) -> Tuple[str, str, bool]:
        """
        Parse and validate a subcategory from Mistral's raw output.
        Returns (category, subcategory, is_confident).

        The CATEGORY is returned too because step 4 below can correct it. If the
        model names a subcategory that genuinely exists but under a different
        parent, the subcategory is the more specific and more reliable signal,
        so the category is moved to match rather than the subcategory being
        thrown away.

        Matching runs in order, stopping at the first hit:
          1. exact
          2. case-insensitive
          3. [FIX-09] word-order-insensitive within the same category
          4. [FIX-10] exact or word-order match under a DIFFERENT category
          5. fuzzy (difflib, cutoff 0.75)
          6. give up: first subcategory, flagged needs_review

        Steps 3 and 4 were added after a dry run over 200 images produced 14
        subcategory warnings, of which 'Abstract Geometric' (the taxonomy has
        'Geometric Abstract') and 'Abstract Drawing' (real, but under Abstract
        while the model had chosen Drawings) were both recoverable and were
        being discarded. The remaining 12 were names the model composed from
        the caption and do not exist anywhere, which is a taxonomy gap rather
        than something code can fix.
        """
        ...
        if sub:
            ...
            if sub in valid_subs:
                return category, sub, True

            # 2. Case-insensitive match
            key = (category, sub.lower())
            if key in _SUB_LOWER:
                return category, _SUB_LOWER[key], True

            # 3. [FIX-09] Word-order-insensitive match inside this category.
            #    difflib scores 'Abstract Geometric' against 'Geometric Abstract'
            #    below the 0.75 cutoff because the characters are in a different
            #    order, even though the two are plainly the same label.
            tk = _tokenkey(sub)
            hit = _SUB_TOKENS.get((category, tk))
            if hit:
                self.logger.debug(f"Subcategory word-order: '{sub}' -> '{hit}' (cat={category})")
                return category, hit, True

            # 4. [FIX-10] The name is valid, but under a different parent.
            #    Trust the subcategory and correct the category. A subcategory
            #    is a much narrower claim than a category, so when the two
            #    disagree the subcategory is the better evidence.
            owner = _SUB_OWNER.get(sub) or _SUB_OWNER.get(_SUB_TOKEN_OWNER.get(tk, ""))
            if owner and owner != category:
                real = sub if sub in TAXONOMY[owner] else _SUB_TOKEN_OWNER_NAME.get(tk)
                if real and real in TAXONOMY[owner]:
                    self.logger.info(
                        f"[FIX] Subcategory '{real}' belongs to {owner}, not {category} "
                        f"— moving the image to {owner}"
                    )
                    return owner, real, True

            # 5. Fuzzy match (cutoff 0.75 — strict enough to avoid bad matches)
            matches = difflib.get_close_matches(sub, valid_subs, n=1, cutoff=0.75)
            if matches:
                self.logger.debug(
                    f"Subcategory fuzzy: '{sub}' → '{matches[0]}' (cat={category})"
                )
                return category, matches[0], True
        ...
        # 6. No usable output — use first subcategory and flag for review
        return category, valid_subs[0], False
    ...
    def classify_captions_batch(
        ...
        Returns list of (category, subcategory, is_confident, category_failed).
        category_failed is True when the model named a category that is not in
        the taxonomy at all, so the caller can decide what to do about it
        instead of silently accepting the Backgrounds fallback.
        ...
        try:
            ...
            for caption in captions:
                try:
                    results.append(self._single_classify(caption))
                except Exception as e2:
                    self.logger.error(f"[FAIL] Sequential classify failed: {e2}")
                    results.append(("Backgrounds", TAXONOMY["Backgrounds"][0], False, True))
            return results
    ...
    def _batch_classify(self, captions: List[str]) -> List[Tuple[str, str, bool, bool]]:
        ...
        for r, (cat, cat_conf) in zip(sub_raws, cat_results):
            # _parse_subcategory may correct the category (see FIX-10)
            cat, sub, sub_conf = self._parse_subcategory(r, cat)
            # [FIX-03] a row is only "confident" when BOTH levels parsed cleanly
            # [FIX-11] cat_conf is reported separately so the re-classify pass
            # can refuse to MOVE an image on the strength of a failed parse.
            results.append((cat, sub, cat_conf and sub_conf, not cat_conf))
        return results

    def _single_classify(self, caption: str) -> Tuple[str, str, bool, bool]:
        n_tok = self.config.MISTRAL_MAX_NEW_TOKENS
        # Step 1
        ...
        category, sub, sub_conf = self._parse_subcategory(sub_raw, category)
        return category, sub, cat_conf and sub_conf, not cat_conf
...
class Pipeline:
            ...
            def classify_all(items):
                ...
                for i in range(0, len(items), mistral_bs):
                    ...
                    try:
                        ...
                        for (key, raw), (cat, sub_cat, conf, _cat_failed), tags in zip(
    ...
    async def run_reclassify(self):
        ...
        review   = 0
        failed   = 0
        kept_on_failure = 0          # [FIX-11] rows left where they were
        moves: Dict[str, int] = {}
        ...
        try:
            while done < target:
                ...
                for img_id, old_cat, (new_cat, new_sub, confident, cat_failed) in zip(
                    ids, old_cats, results
                ):
                    # [FIX-11] The model sometimes names a category that is not in
                    # the taxonomy at all ('Artwork', 'Cityscape', 'Smoke & Fog',
                    # 'Clock & Watches' all appeared in the 200-image dry run).
                    # _parse_category answers 'Backgrounds' in that case, which is
                    # a sane default for a NEW image but actively harmful here: it
                    # would move an existing image out of its current category on
                    # the strength of a parse that failed. Keep what the row
                    # already has and flag it instead. The needs_review status is
                    # the useful output, not the move.
                    if cat_failed and old_cat:
                        new_cat  = old_cat
                        new_sub  = None
                        confident = False
                        kept_on_failure += 1
        ...
        self.logger.info(f"  Kept (bad cat): {kept_on_failure:,}   "
                         f"(model named a category not in the taxonomy — "
                         f"left in place, flagged needs_review)")
